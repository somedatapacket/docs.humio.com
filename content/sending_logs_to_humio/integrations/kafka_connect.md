...
In this example, we assume Humio and Kafka is already running. We will use Docker and the [confluentinc/cp-kafka-connect](https://hub.docker.com/r/confluentinc/cp-kafka-connect/) image as it has the [Confluent Elasticsearch connector](https://docs.confluent.io/current/connect/connect-elasticsearch/docs/index.html).
...
docker run -i -t \
  ...
  --rm=true \
  -v $(pwd):/config \
  --add-host kafka:$KAFKA \
  confluentinc/cp-kafka-connect: connect-distributed /config/worker.properties

```

Insert the ip address of a Kafka server in `$KAFKA` and the above script will start tREST API for managing connectors on localhost port 10082.
