...
```
GET     /api/v1/repos/$REPOSITORY_NAME/parsers
```
...
``` shell
curl https://demo.humio.com/api/v1/repos/$REPOSITORY_NAME/parsers \
...
```
POST     /api/v1/repos/$REPOSITORY_NAME/parsers/$PARSER_ID
```

or to updated an existing parser use

```
PUT     /api/v1/repos/$REPOSITORY_NAME/parsers/$PARSER_ID
```
...
``` shell
curl https://demo.humio.com/api/v1/repos/$REPOSITORY_NAME/parsers/$PARSER_NAME \
 -XPUT \
 -H "Authorization: Bearer $API_TOKEN" \
...
```
DELETE     /api/v1/repos/$REPOSITORY_NAME/parsers/$PARSER_NAME
```
